...
export const Video = db => ({
  create: async ({
    uploaderID, title, description, keywords, filename, filetype, filesize, md5Hash, metadata 
  }) => (await db
    .insert(videos)
    .values({uploaderID, title, description, keywords, filename, filetype, filesize, md5Hash, metadata})
    .onConflictDoNothing()
    .returning()
  )[0],
